<template>
  <div class="homeDetalhado">
    ...
    <b-row class="mt-4 mb-4">
      ...
      <b-col class="end">
        <router-link to="/cliente/adicionar">
    ...
    <b-table
      ...
      :filter="searchText"
    >
      <template v-slot:cell(acoes)="dataClientes" class="botoes">
        <div>
          <button class="button" @click="mudarRota(dataClientes.item)">
        ...
        <div>
          <button
            class="button"
            @click="abrirModalClientes(dataClientes.item._id)"
          >
    ...
    <ModalExcluir :confirma_acao="excluir" texto="cliente" />
  </div>
</template>

<script>
import { getCliente, excluirCliente } from "@/services/api/Cliente.js";
import ModalExcluir from "@/components/modalExcluirCliente";
export default {
  ...
  methods: {
    ...
    abrirModalClientes(id) {
      this.id = id;
      this.$bvModal.show("modal-excluir-cliente");
    },

    mudarRota(data) {
      this.$router.push(`/clientes/editar/${data._id}`);
    },

    excluir() {
      excluirCliente(this.id)
        .then(() => {
          this.$bvModal.hide("modal-excluir-cliente");
          this.receberDados();
