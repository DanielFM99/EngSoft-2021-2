<template>
  <div class="homeDetalhado">
    ...
    <b-row class="mt-4 mb-4">
      ...
      <b-col class="end">
        <router-link to="/funcionario/adicionar">
    ...
    <b-table
      ...
      :filter="searchText"
    >
      <template v-slot:cell(acoes)="dataFuncionarios" class="botoes">
        <div>
          <button class="button" @click="mudarRota(dataFuncionarios.item)">
        ...
        <div>
          <button
            class="button"
            @click="abrirModalFuncionario(dataFuncionarios.item._id)"
          >
...
export default {
  ...
  methods: {
    abrirModalFuncionario(id) {
      this.id = id;
      this.$bvModal.show("modal-excluir-funcionario");
    },
    receberDados() {
      getFuncionario()
        .then((res) => {
    ...
    mudarRota(data) {
      this.$router.push(`/funcionarios/editar/${data._id}`);
    },
    excluir() {
      excluirFuncionario(this.id)
        .then(() => {
          this.$bvModal.hide("modal-excluir-funcionario");
          this.receberDados();
